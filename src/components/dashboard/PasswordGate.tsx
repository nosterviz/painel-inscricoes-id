    ...
    <div className="min-h-screen bg-[#050A0F] flex items-center justify-center p-4">
      ...
      <div className="max-w-md w-full relative">
        <div className="flex flex-col items-center mb-8">
          <div className="w-16 h-16 bg-[#111827] border border-white/5 rounded-2xl flex items-center justify-center mb-6 shadow-xl">
            <Lock className="w-8 h-8 text-[#2BBDCE]" />
          </div>
          <div className="text-center">
            <span className="font-mono text-[10px] tracking-[0.2em] text-[#2BBDCE] uppercase block mb-2">O GRANDE FILTRO</span>
        ...
        <form onSubmit={handleLogin} className="bg-[#111827] border border-white/5 p-8 rounded-2xl shadow-2xl space-y-6 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-[#2BBDCE]/20" />
          <div>
            <label className="block font-mono text-[10px] uppercase text-zinc-500 mb-2 tracking-wide">Senha de Acesso</label>
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Digite a senha..." className="w-full bg-[#050A0F] border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-zinc-700 focus:outline-none focus:border-[#2BBDCE]/50 transition-colors" autoFocus />
          ...
          <button type="submit" disabled={!password || isChecking} className="w-full bg-[#2BBDCE] hover:bg-[#34d2e4] disabled:bg-zinc-800 disabled:text-zinc-600 text-[#050A0F] font-bold py-3 rounded-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2">
